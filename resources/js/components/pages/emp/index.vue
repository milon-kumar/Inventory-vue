<template>
    <div>
        <div class="card card-custom">
            ...
            <div class="card-body">
                <!--begin: Datatable-->
                <table class="table table-borderless table-striped" id="kt_datatable">
                    <thead>
                    <tr>
                        <th>Record ID</th>
                        <th>Order ID</th>
                        ...
                        <form class="form" @submit.prevent="saveEmployee()" enctype="multipart/form-data">
                            <div class="card-body">
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea id="exampleTextarea" class="form-control form-control-solid caegroyTextArea" v-model="from.address" rows="3"></textarea>
                                </div>

                                <img  :src="imagePage" alt="" class="placeholder__image">
                                <div class="form-group">
                                    <label class="btn btn-clean file__upload" >
...
export default {
    ...
    data() {
        return {
            ...
            imagePage:"http://www.vvc.cl/wp-content/uploads/2016/09/ef3-placeholder-image.jpg",
    ...
    methods: {
        ...
        uploadFile(event){
            ...
            rider.onload = event =>{
                this.imagePage = event.target.result;
            }
            rider.readAsDataURL(this.from.photo);

        },
        clearForm(){
            this.from = '';
            this.imagePage =  'http://www.vvc.cl/wp-content/uploads/2016/09/ef3-placeholder-image.jpg';
        },
        async saveEmployee(){
            let formdata = new FormData();
            this.from.add  = document.getElementById('exampleTextarea').value;
            console.log(this.from.add);
            ...
            formdata.append('address',  this.from.add);
            formdata.append('photo',    this.from.photo);


            await axios.post('api/save/emp', formdata)
                       .then(res=>{
                           Toast.fire({
                               icon: 'success',
...
.placeholder__image{
    ...
    margin-bottom: 1rem;
    border-radius: 12px;
    box-shadow: 0px 3px 7px -2px #d1d1d1;
}
